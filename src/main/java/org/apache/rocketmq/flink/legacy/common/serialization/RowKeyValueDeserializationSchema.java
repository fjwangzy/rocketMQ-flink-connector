...
/**
 * * The row based implementation of {@link KeyValueDeserializationSchema} for the
 * deserialization of message key and value..
 */
public class RowKeyValueDeserializationSchema implements KeyValueDeserializationSchema<RowData> {

	private static final long serialVersionUID = -1L;

	private static final Logger LOGGER = LoggerFactory.getLogger(RowKeyValueDeserializationSchema.class);
	...
	public RowKeyValueDeserializationSchema(TableSchema tableSchema, DirtyDataStrategy formatErrorStrategy,
			DirtyDataStrategy fieldMissingStrategy, DirtyDataStrategy fieldIncrementStrategy, String encoding,
			String fieldDelimiter, boolean columnErrorDebug, Map<String, String> properties) {
		...
		for (int index = 0; index < columnSize; index++) {
			ByteSerializer.ValueType type = ByteSerializer
				.getTypeIndex(tableSchema.getFieldTypes()[index].getTypeClass());
	...
	private boolean isByteArrayType(String fieldName) {
		...
		if (typeInformation != null) {
			ByteSerializer.ValueType valueType = ByteSerializer.getTypeIndex(typeInformation.getTypeClass());
	...
	private String[] handleFieldMissing(String[] data) {
		String fieldMissingMessage = String.format(
	...
	private String[] handleFieldIncrement(String[] data) {
		String fieldIncrementMessage = String.format(
	...
	public static class Builder {
		...
		public RowKeyValueDeserializationSchema build() {
			return new RowKeyValueDeserializationSchema(schema, formatErrorStrategy, fieldMissingStrategy,
					fieldIncrementStrategy, encoding, fieldDelimiter, columnErrorDebug, properties);
		}
