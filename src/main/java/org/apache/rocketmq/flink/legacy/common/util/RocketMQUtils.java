...
public final class RocketMQUtils {
	 ...
	 */
	public static List<MessageQueue> allocate(Collection<MessageQueue> mqSet, int numberOfParallelTasks,
			int indexOfThisTask) {
	...
	public static Map<MessageQueue, Long> initOffsets(List<MessageQueue> messageQueues, DefaultMQPullConsumer consumer,
			StartupMode startMode, OffsetResetStrategy offsetResetStrategy, long specificTimeStamp,
			Map<MessageQueue, Long> specificStartupOffsets) throws MQClientException {
		...
		for (MessageQueue mq : messageQueues) {
			...
			switch (startMode) {
				...
				case GROUP_OFFSETS:
					...
					// If broker throw exception,return -2.should be distinguished from
					// the
					...
					// the min offset return if consumer group first join,return a
					// negative number
					...
					if (offset <= 0) {
						switch (offsetResetStrategy) {
							case LATEST:
								offset = consumer.maxOffset(mq);
								log.info("current consumer thread:{} has no committed offset,use Strategy:{} instead",
										mq, offsetResetStrategy);
								break;
							case EARLIEST:
								log.info("current consumer thread:{} has no committed offset,use Strategy:{} instead",
										mq, offsetResetStrategy);
				...
				case SPECIFIC_OFFSETS:
					...
					break;
				default:
					throw new IllegalArgumentException("current startMode is not supported" + startMode);
			}
			log.info("current consumer queue:{} start from offset of: {}", mq.getBrokerName() + "-" + mq.getQueueId(),
					offset);
