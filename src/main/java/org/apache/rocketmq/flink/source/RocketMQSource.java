...
public class RocketMQSource<OUT>
	...
	public RocketMQSource(String topic, String consumerGroup, String nameServerAddress, String accessKey,
			String secretKey, String tag, String sql, long stopInMs, long partitionDiscoveryIntervalMs,
			Boundedness boundedness, RocketMQDeserializationSchema<OUT> deserializationSchema, StartupMode startMode,
		...
		Validate.isTrue(!(boundedness == Boundedness.BOUNDED && partitionDiscoveryIntervalMs > 0),
	...
	public SourceReader<OUT, RocketMQPartitionSplit> createReader(SourceReaderContext readerContext) throws Exception {
		...
		Supplier<SplitReader<Tuple3<OUT, Long, Long>, RocketMQPartitionSplit>> splitReaderSupplier = () -> new RocketMQPartitionSplitReader<>(
				topic, consumerGroup, nameServerAddress, accessKey, secretKey, tag, sql, deserializationSchema,
				readerContext, commitOffsetAuto);
		RocketMQRecordEmitter<OUT> recordEmitter = new RocketMQRecordEmitter<>();

		return new RocketMQSourceReader<>(elementsQueue,
				new RocketMQSourceFetcherManager<>(elementsQueue, splitReaderSupplier, (ignore) -> {
				}), recordEmitter, new Configuration(), readerContext);
	}
	...
	public SplitEnumerator<RocketMQPartitionSplit, RocketMQSourceEnumState> createEnumerator(
		...
		return new RocketMQSourceEnumerator(topic, consumerGroup, nameServerAddress, accessKey, secretKey, stopInMs,
				partitionDiscoveryIntervalMs, boundedness, enumContext, startMode, offsetResetStrategy, specificOffsets,
				consumerOffsetTimestamp);
	}

	@Override
	public SplitEnumerator<RocketMQPartitionSplit, RocketMQSourceEnumState> restoreEnumerator(
			SplitEnumeratorContext<RocketMQPartitionSplit> enumContext, RocketMQSourceEnumState checkpoint) {
		...
		return new RocketMQSourceEnumerator(topic, consumerGroup, nameServerAddress, accessKey, secretKey, stopInMs,
				partitionDiscoveryIntervalMs, boundedness, enumContext, checkpoint.getCurrentAssignment(), startMode,
				offsetResetStrategy, specificOffsets, consumerOffsetTimestamp);
	}
	...
	Configuration getConfiguration() {
		...
		conf.set(RocketMQOptions.OPTIONAL_SCAN_STARTUP_SPECIFIC_OFFSETS, specificOffsets.toString());
		return conf;
	}
