...
/**
 * The @builder class for {@link RocketMQSource}to make it easier for the users to
 * construct a {@link RocketMQSource}.
 */
public class RocketMQSourceBuilder<OUT> {

	private static final String[] REQUIRED_CONFIGS = { RocketMQConfig.NAME_SERVER_ADDR, RocketMQConfig.CONSUMER_GROUP,
	...
	public RocketMQSourceBuilder() {
		...
		this.partitionDiscoveryIntervalMs = RocketMQOptions.OPTIONAL_PARTITION_DISCOVERY_INTERVAL_MS.defaultValue();
	...
	public RocketMQSourceBuilder<OUT> setDeserializer(RocketMQDeserializationSchema<OUT> deserializationSchema) {
	...
	public RocketMQSourceBuilder<OUT> setPartitionDiscoveryIntervalMs(long partitionDiscoveryIntervalMs) {
	 ...
	 */
	public RocketMQSourceBuilder<OUT> setStartFromGroupOffsets(OffsetResetStrategy offsetResetStrategy) {
		this.startMode = StartupMode.GROUP_OFFSETS;
	 ...
	 * specify offset.
	 */
	public RocketMQSourceBuilder<OUT> setStartFromSpecificOffsets(Map<MessageQueue, Long> specificOffsets) {
	...
	public RocketMQSource<OUT> build() {
		sanityCheck();
		return new RocketMQSource(props.getProperty(RocketMQConfig.CONSUMER_TOPIC),
				props.getProperty(RocketMQConfig.CONSUMER_GROUP), props.getProperty(RocketMQConfig.NAME_SERVER_ADDR),
				props.getProperty(RocketMQConfig.ACCESS_KEY), props.getProperty(RocketMQConfig.SECRET_KEY),
				props.getProperty(RocketMQConfig.CONSUMER_TAG), props.getProperty(RocketMQConfig.CONSUMER_SQL),
				stopInMs, partitionDiscoveryIntervalMs, boundedness, deserializationSchema, startMode,
				offsetResetStrategy, specificStartupOffsets, consumerOffsetTimestamp, commitOffsetAuto);
	}

	private void sanityCheck() {
		for (String requiredConfig : REQUIRED_CONFIGS) {
			Validate.notNull(props.getProperty(requiredConfig),
