...
/**
 * The {@link SimpleVersionedSerializer Serializer} for the enumerator state of RocketMQ
 * source.
 */
public class RocketMQSourceEnumStateSerializer implements SimpleVersionedSerializer<RocketMQSourceEnumState> {
	...
	public byte[] serialize(RocketMQSourceEnumState enumState) throws IOException {
		return SerdeUtils.serializeSplitAssignments(enumState.getCurrentAssignment(),
	...
	public RocketMQSourceEnumState deserialize(int version, byte[] serialized) throws IOException {
		...
		if (version == getVersion()) {
			Map<Integer, List<RocketMQPartitionSplit>> currentPartitionAssignment = SerdeUtils
				.deserializeSplitAssignments(serialized, new RocketMQPartitionSplitSerializer(), ArrayList::new);
