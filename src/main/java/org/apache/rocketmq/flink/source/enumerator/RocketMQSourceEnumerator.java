...
public class RocketMQSourceEnumerator implements SplitEnumerator<RocketMQPartitionSplit, RocketMQSourceEnumState> {
	...
	/**
	 * This set is only accessed by the partition discovery callable in the callAsync()
	 * method, i.e worker thread.
	 */
	...
	/**
	 * The discovered and initialized partition splits that are waiting for owner reader
	 * to be ready.
	 */
	...
	public RocketMQSourceEnumerator(String topic, String consumerGroup, String nameServerAddress, String accessKey,
			String secretKey, long stopInMs, long partitionDiscoveryIntervalMs, Boundedness boundedness,
			SplitEnumeratorContext<RocketMQPartitionSplit> context, StartupMode startMode,
			OffsetResetStrategy offsetResetStrategy, Map<MessageQueue, Long> specificStartupOffsets,
			long consumerOffsetTimestamp) {
		this(topic, consumerGroup, nameServerAddress, accessKey, secretKey, stopInMs, partitionDiscoveryIntervalMs,
				boundedness, context, new HashMap<>(), startMode, offsetResetStrategy, specificStartupOffsets,
	...
	public RocketMQSourceEnumerator(String topic, String consumerGroup, String nameServerAddress, String accessKey,
			...
			Map<Integer, List<RocketMQPartitionSplit>> currentSplitsAssignments, StartupMode startMode,
			OffsetResetStrategy offsetResetStrategy, Map<MessageQueue, Long> specificStartupOffsets,
			long consumerOffsetTimestamp) {
		this.topic = topic;
		...
		this.readerIdToSplitAssignments.forEach((reader, splits) -> splits.forEach(s -> {
			// Recover offset from checkpoint
			offsetTable.put(new MessageQueue(s.getTopic(), s.getBroker(), s.getPartition()), s.getStartingOffset());
			// Recover discovered partitions from checkpoint
			discoveredPartitions.add(new Tuple3<>(s.getTopic(), s.getBroker(), s.getPartition()));
		}));
	...
	public void start() {
		...
		if (partitionDiscoveryIntervalMs > 0) {
			LOG.info(
					...
					consumerGroup, partitionDiscoveryIntervalMs);
			context.callAsync(this::discoverAndInitializePartitionSplit, this::handlePartitionSplitChanges, 0,
					partitionDiscoveryIntervalMs);
		}
		else {
			LOG.info("Starting the RocketMQSourceEnumerator for consumer group {} "
					+ "without periodic partition discovery.", consumerGroup);
	...
	public void addReader(int subtaskId) {
		LOG.debug("Adding reader {} to RocketMQSourceEnumerator for consumer group {}.", subtaskId, consumerGroup);
		assignPendingPartitionSplits(Collections.singleton(subtaskId));
	}
	...
	private Set<RocketMQPartitionSplit> discoverAndInitializePartitionSplit() throws MQClientException {
		...
		for (MessageQueue messageQueue : messageQueues) {
			...
			if (!removedPartitions.remove(topicPartition)) {
				...
				result.add(new RocketMQPartitionSplit(topicPartition.f0, topicPartition.f1, topicPartition.f2,
						offsetTable.get(messageQueue), stopInMs));
			}
		}
	...
	private void addPartitionSplitChangeToPendingAssignments(Collection<RocketMQPartitionSplit> newPartitionSplits) {
		...
		for (RocketMQPartitionSplit split : newPartitionSplits) {
			int ownerReader = getSplitOwner(split.getTopic(), split.getBroker(), split.getPartition(), numReaders);
			pendingPartitionSplitAssignment.computeIfAbsent(ownerReader, r -> new HashSet<>()).add(split);
		}
		LOG.debug("Assigned {} to {} readers of consumer group {}.", newPartitionSplits, numReaders, consumerGroup);
	}
	...
	private void assignPendingPartitionSplits(Set<Integer> pendingReaders) {
		...
		// to be assigned.Whether bounded or unbounded flow，the idle subtask will be
		// released.
		if (noMoreNewPartitionSplits) {
			LOG.debug("No more RocketMQPartitionSplits to assign. Sending NoMoreSplitsEvent to the readers "
					+ "in consumer group {}.", consumerGroup);
	...
	private void initOffsets(Set<MessageQueue> mqSets) {
		try {
			Map<MessageQueue, Long> map = RocketMQUtils.initOffsets(new ArrayList<>(mqSets), consumer, startMode,
					offsetResetStrategy, consumerOffsetTimestamp, specificStartupOffsets);
	...
	/**
	 * Returns the index of the target subtask that a specific RocketMQ partition should
	 * be assigned to.
	 *
	 * <p>
	 * The resulting distribution of partitions of a single topic has the following
	 * contract:
	 *
	 * <ul>
	...
	static int getSplitOwner(String topic, String broker, int partition, int numReaders) {
		...
		// starting from 0, and therefore can be used directly as the offset clockwise
		// from the
		// start index
