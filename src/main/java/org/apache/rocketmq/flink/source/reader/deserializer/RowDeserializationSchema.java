...
/**
 * The row based implementation of {@link DeserializationSchema} for the deserialization
 * of records.
 */
public class RowDeserializationSchema implements DeserializationSchema<List<BytesMessage>, RowData> {
	...
	public RowDeserializationSchema(TableSchema tableSchema, DirtyDataStrategy formatErrorStrategy,
			DirtyDataStrategy fieldMissingStrategy, DirtyDataStrategy fieldIncrementStrategy, String encoding,
			String fieldDelimiter, String lineDelimiter, boolean columnErrorDebug, boolean hasMetadata,
			MetadataConverter[] metadataConverters, List<String> headerFields, Map<String, String> properties) {
		...
		for (int index = 0; index < totalColumnSize; index++) {
			ValueType type = ByteSerializer.getTypeIndex(tableSchema.getFieldTypes()[index].getTypeClass());
		...
		for (BytesMessage message : messages) {
			...
			if (isOnlyHaveVarbinaryDataField()) {
				...
				for (int index = 0; index < totalColumnSize; index++) {
					...
					rowData.setField(index, StringSerializer.deserialize(headerValue, fieldTypes[index],
							fieldDataTypes[index], new HashSet<>()));
				}
		...
		for (String line : lines) {
			...
			for (int index = 0; index < totalColumnSize; index++) {
				try {
					...
					rowData.setField(index, StringSerializer.deserialize(fieldValue, fieldTypes[index],
							fieldDataTypes[index], new HashSet<>()));
				}
				catch (Exception e) {
	...
	private boolean isByteArrayType(String fieldName) {
		TypeInformation<?> typeInformation = tableSchema.getFieldTypes()[columnIndexMapping.get(fieldName)];
		...
		switch (formatErrorStrategy) {
			case SKIP:
				...
				if (columnErrorDebug || now - lastLogExceptionTime > DEFAULT_LOG_INTERVAL_MS) {
					LOGGER.warn("Data format error, field type: " + fieldTypes[index] + "field data: " + data[index]
							+ ", index: " + index + ", data: [" + StringUtils.join(data, ",") + "]", e);
		...
		switch (fieldMissingStrategy) {
			case SKIP:
				...
				if (columnErrorDebug || now - lastLogHandleFieldTime > DEFAULT_LOG_INTERVAL_MS) {
					LOGGER.warn("Field missing error, table column number: " + totalColumnSize
							+ ", data column number: " + dataColumnSize + ", data field number: " + data.length
	...
	private String[] handleFieldIncrement(String[] data) {
		switch (fieldIncrementStrategy) {
			case SKIP:
				long now = System.currentTimeMillis();
				if (columnErrorDebug || now - lastLogHandleFieldTime > DEFAULT_LOG_INTERVAL_MS) {
					LOGGER.warn("Field increment error, table column number: " + totalColumnSize
							+ ", data column number: " + dataColumnSize + ", data field number: " + data.length
							+ ", data: [" + StringUtils.join(data, ",") + "]");
					lastLogHandleFieldTime = now;
				}
	...
	public static final class MetadataCollector implements Collector<RowData>, Serializable {
		...
		public void collect(RowData physicalRow) {
			if (hasMetadata) {
				...
				final GenericRowData producedRow = new GenericRowData(physicalRow.getRowKind(),
				...
				for (int index = 0; index < metadataArity; index++) {
					producedRow.setField(index + physicalArity, metadataConverters[index].read(message));
				}
	...
	public static class Builder {
		...
		public RowDeserializationSchema build() {
			return new RowDeserializationSchema(schema, formatErrorStrategy, fieldMissingStrategy,
					fieldIncrementStrategy, encoding, fieldDelimiter, lineDelimiter, columnErrorDebug, hasMetadata,
					metadataConverters, headerFields, properties);
		}
	...
	public static class CollectorOption {
		...
		public static final ConfigOption<String> ENCODING = ConfigOptions.key("encoding".toLowerCase())
			.defaultValue("UTF-8");

		public static final ConfigOption<String> FIELD_DELIMITER = ConfigOptions.key("fieldDelimiter".toLowerCase())
			.defaultValue("\u0001");

		public static final ConfigOption<String> LINE_DELIMITER = ConfigOptions.key("lineDelimiter".toLowerCase())
			.defaultValue("\n");
		...
		public static final ConfigOption<String> LENGTH_CHECK = ConfigOptions.key("lengthCheck".toLowerCase())
			.defaultValue("NONE");
