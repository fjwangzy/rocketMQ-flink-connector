...
public class RocketMQOptionsUtil {
	 ...
	 */
	public static Map<MessageQueue, Long> parseSpecificOffsets(String specificOffsetsStr, String topicName) {
		...
		for (String pair : pairs) {
			...
			if (kv.length != 3 || !kv[0].startsWith(BROKER + ':') || !kv[1].startsWith(QUEUE + ':')
	...
	public static void validateStartUpMode(Configuration configuration) {
		configuration.getOptional(OPTIONAL_SCAN_STARTUP_MODE).ifPresent(mode -> {
			switch (mode) {
				case SPECIFIC_OFFSETS:
					if (!configuration.getOptional(OPTIONAL_SCAN_STARTUP_SPECIFIC_OFFSETS).isPresent()) {
						throw new ValidationException(
								String.format("'%s' is required in '%s' startup mode" + " but missing.",
										OPTIONAL_SCAN_STARTUP_SPECIFIC_OFFSETS.key(), StartupMode.SPECIFIC_OFFSETS));
					}
					String specificOffsets = configuration.getString(OPTIONAL_SCAN_STARTUP_SPECIFIC_OFFSETS);
					String topic = configuration.getString(TOPIC);
					RocketMQOptionsUtil.parseSpecificOffsets(specificOffsets, topic);
					break;
				case TIMESTAMP:
					if (!configuration.getOptional(OPTIONAL_SCAN_STARTUP_TIMESTAMP_MILLIS).isPresent()) {
						throw new ValidationException(
								String.format("'%s' is required in '%s' startup mode" + " but missing.",
										OPTIONAL_SCAN_STARTUP_TIMESTAMP_MILLIS.key(), StartupMode.TIMESTAMP));
					}
					break;
				default:
					break;
